...
for folder in folder_list:
	...
	if len(files) >= 201:
		...
		number_files.append(str(len(files)-1))
...
for i in range(len(number_files)):
	if i == len(number_files) - 1:
		...
		labels[start:(start+int(number_files[i]))] = i
...
with open('out.txt','a') as f:
	f.write( 'model built and compiled\n')
	f.write(str(time.time()))
f.close()
# Training
...
with open('out.txt','a') as f:
	f.write( 'Testing Ends\n')
	f.write(str(time.time()))
f.close()
print(acc/float(coun))
with open('out.txt','a') as f:
	f.write(str(acc/float(coun)))
f.close()
